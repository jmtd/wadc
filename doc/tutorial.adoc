...
== Now for a corridor (and an annoying bug)
...
Hmm it didn't work. Why? Well, this is a long standing bug/behaviour in WadC
that I haven't managed to fix or replace yet. When trying to define the sector,
it's looking at a segment of a previously drawn line and complaining that it
has already assigned that segment to a previously drawn sector (the last boring
room).

Until I've fixed this, the workaround is to tweak our corridor routine to draw
things in a different order.

.tutorial/14.wl
----
include::tutorial/14.wl[]
----
image::tutorial/14.png[title="14.wad"]

So here we are drawing the corridor backwards. We first move inwards a bit,
because we want the corridor inset; just as before. We then move to where the
opposite corner of the corridor will be; turn around, and draw the corner.
Finally we move around again so that we're positioned correctly for future
calls to `corridor`, or to `boring2`.
