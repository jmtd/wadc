...
launch it, you need to run
    ...
    java -cp wadc.jar path/to/input.wl
...
lisp.h::        Basic lisp-style lists.
water.h::       Helper routines for using Boom sector transfer effect
                (linedef type 242) for deep water.
...
http://doomwiki.org/wiki/Friendly_monster#MBF[MBF feature]. 'friendly' defaults
to off.
...
any following monsters will be deaf. Calling it again switches it off again.
