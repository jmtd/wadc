 ...
 * It is now much more convenient to generate maps for the original Doom,
   Heretic and Hexen, in addition to Doom II.
 ...
 * The "pipes" stuff in examples/beta continues to evolve and drive
   WadC development.
 * `doom_ex.wl`, `htic_ex.wl`, `hexen_ex.wl`: very simple test maps that
    demonstrate Doom #1/Heretic/Hexen support
...
`doom.h`, `heretic.h`, `hexen.h`:: sensible defaults and thing definitions
                                   for Doom (#1), Heretic and Hexen
